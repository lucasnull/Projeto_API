...
def get_user_admin(token=Depends(oauth2_schema)):
    try:
...
def get_user(token=Depends(oauth2_schema)):
    ...
    if not username:
        raise HTTPException(status_code=401, detail="Invalid Token!!!")
    # se tiver um username vai verificar se tem no banco
    else:
        db_user = (databaseDAO.select_value(f"SELECT * FROM bancoproj.users WHERE username='{username}'"))

    if len(db_user) == 0:
        raise HTTPException(status_code=400, detail="Has no user on database!!!")
